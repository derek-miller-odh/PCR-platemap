...
try:
    ...
    print('Error entering run number')
reString = r"\.xlsx$"
try:
    flist = []
    ...
    if len(flist) > 1:
        print('Too many excel files found, please rerun with only one excel file')
    elif len(flist) == 0:
...
class RunProgram:
    def __init__(self):
        ...
        try:    
            ...
            print('Run Program class failed to initialize')
...
class MakeExport:
    def __init__(self, _plate):
            ...
            try:
                ...
                self.file = open(f'{rnumber}-1_AR-CRO-PCR-ABI-7500_{self.time} - {self.plate.name}.txt','w')
